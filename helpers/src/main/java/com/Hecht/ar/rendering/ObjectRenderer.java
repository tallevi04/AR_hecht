package com.Hecht.ar.rendering;
...
public class ObjectRenderer {
  ...
  // The last component must be zero to avoid applying the translational part of the matrix.
   ...
   * parameter {context} Context for loading the shader and below-named model and texture assets.
   * parameter {objAssetName} Name of the OBJ file containing the model geometry.
   * parameter {diffuseTextureAssetName} Name of the PNG file containing the diffuse texture map.
   */
   ...
   * parameter {blendMode} The blending mode. Null indicates no blending (opaque rendering).
   */
   ...
   * parameter {modelMatrix} A 4x4 model-to-world transformation matrix, stored in column-major order.
   * parameter {scaleFactor} A separate scaling factor to apply before the {modelMatrix}.
   */
   ...
   * parameter {ambient} Intensity of non-directional surface illumination.
   * parameter {diffuse} Diffuse (matte) surface reflectivity.
   * parameter {specular} Specular (shiny) surface reflectivity.
   * parameter {specularPower} Surface shininess.
   * Larger values result in a smaller, sharper specular highlight.
   */
  ...
  /**
   * Draws the model.
   * parameter {cameraView} A 4x4 view matrix, in column-major order.
   * parameter {cameraPerspective} A 4x4 projection matrix, in column-major order.
   * parameter {lightIntensity} Illumination intensity. Combined with diffuse and specular material properties.
   */
  ...
  public void draw(
    ...
    Matrix.multiplyMM(
            modelViewProjectionMatrix, 0, cameraPerspective, 0, modelViewMatrix, 0);
    GLES20.glUseProgram(program);
